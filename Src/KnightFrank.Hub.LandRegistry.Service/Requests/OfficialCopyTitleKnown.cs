...
{
    // Concrete Creator
    public class OfficialCopyTitleKnownFactory : ServiceFactory
    {
        private readonly X509Certificate2? _clientCertificate;

        public OfficialCopyTitleKnownFactory(IMapper mapper, ILogger<LandRegistrySvc> logger, X509Certificate2? clientCertificate = null)
        {
            _mapper = mapper;
            _logger = logger;
        ...
        {
            return new OfficialCopyTitleKnown(_mapper, _logger);
        }
    }

    // Concrete 
    public class OfficialCopyTitleKnown : LandRegistry
    {
        private readonly IMapper _mapper;
        private readonly ILogger<LandRegistrySvc> _logger;
        private readonly X509Certificate2? _clientCertificate;
        private LandRegistryDto _dto;
        private RequestTitleKnownOfficialCopyV2_1Type _request;
        private performTitleKnownSearchResponse _response;

        public OfficialCopyTitleKnown(IMapper mapper, ILogger<LandRegistrySvc> logger, X509Certificate2? clientCertificate = null)
        {
            _mapper = mapper;
            _logger = logger;
            _clientCertificate = clientCertificate;
        ...
        public override void Map(LandRegistryDto landRegistryDto)
        {
            ...
            // Map JSON to XML
            _request = _mapper.Map<RequestTitleKnownOfficialCopyV2_1Type>(_dto);
        ...
        public override async Task Request()
        {
            ...
            _response = await client.performTitleKnownSearchAsync(_request);
            //client.CloseAsync();
        }
        ...
        public override LandRegistryDto Response()
        {
            ...
            return _mapper.Map<performTitleKnownSearchResponse, LandRegistryDto>(_response, _dto);
        }

        private OC1TitleKnownV2_1ServiceClient GetWsClient()
        {
            ...
            var client = new OC1TitleKnownV2_1ServiceClient(Binding(), new EndpointAddress(Environment.GetEnvironmentVariable("LandRegistryApplicationEnquiry")));
