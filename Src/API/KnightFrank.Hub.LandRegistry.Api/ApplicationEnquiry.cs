...
{
    public class ApplicationEnquiry(ILogger<ApplicationEnquiry> log, IMapper mapper, ILandRegistrySvc hmlr)
    {
        private readonly ILogger<ApplicationEnquiry> _logger = log;
        private readonly IMapper _mapper = mapper;
        private readonly ILandRegistrySvc _hmlr = hmlr;

        [Function("ApplicationEnquiry")]
        [OpenApiOperation(operationId: "ApplicationEnquiryReq", tags: new[] { "name" })]
        [OpenApiRequestBody(contentType: "application/json", bodyType: typeof(ApplicationEnquiryReq))]
        ...
        public async Task<HttpResponseData> ApplicationEnquiryReq([HttpTrigger(AuthorizationLevel.Anonymous, "post")] HttpRequestData req)
        {
            _logger.LogInformation("Application Enquiry  request.");

            var response = req.CreateResponse();
            try
            {
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync();
                var data = JsonConvert.DeserializeObject<ApplicationEnquiryReq>(requestBody);
