...
{
    public class OfficialSearchPart(ILogger<OfficialSearchPart> log, IMapper mapper, ILandRegistrySvc hmlr)
    {
        private readonly ILogger<OfficialSearchPart> _logger = log;
        private readonly IMapper _mapper = mapper;
        private readonly ILandRegistrySvc _hmlr = hmlr;

        [Function("OfficialSearchPart")]
        [OpenApiOperation(operationId: "OfficialSearchPartReq", tags: new[] { "name" })]
        [OpenApiRequestBody(contentType: "application/json", bodyType: typeof(OfficialSearchPartReq))]
        ...
        public async Task<HttpResponseData> OfficialSearchPartReq([HttpTrigger(AuthorizationLevel.Anonymous, "post", Route = null)] HttpRequestData req)
        {
            _logger.LogInformation("OfficialSearchPart request.");

            var response = req.CreateResponse();
            try
            {
                string requestBody = await new StreamReader(req.Body).ReadToEndAsync();
                var data = JsonConvert.DeserializeObject<OfficialSearchPartReq>(requestBody);
            ...
            catch (Newtonsoft.Json.JsonReaderException ex)
            {
                ...
                response.StatusCode = HttpStatusCode.BadRequest;
                response.Headers.Add("Content-Type", "text/plain; charset=utf-8");
                response.WriteString($"Invalid Json at line {ex.LineNumber} position {ex.LinePosition}");
                return response;
            }
            catch (Exception ex)
            {
